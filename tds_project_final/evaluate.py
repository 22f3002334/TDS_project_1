...
async def a2(email: str, file: str = "/data/format.md", **kwargs):
    ...
    # Run Prettier to get expected output
    process = await asyncio.create_subprocess_exec(
        "npx", "prettier@3.4.2", "--stdin-filepath", file,
        stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    stdout, stderr = await process.communicate(input=original.encode())

    if process.returncode != 0:
        raise RuntimeError(f"Prettier error: {stderr.decode()}")

    expected = stdout.decode()

    # Run LLM-generated formatting command
